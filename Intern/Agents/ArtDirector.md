...
**Visual reference rule**: for Stackspire, read `Docs/Visual-Design.md` before any future generation prompt and treat it as the required visual reference alongside `GDD.md`.

**Perspective rule**: Stackspire MVP art uses three-quarter top-down perspective. Do not generate strict top-down final art unless a task explicitly identifies it as a temporary implementation placeholder.

**Interactive UI state bundle rule**: interactive UI assets should be generated with all required states together in one prompt/output set whenever practical, so default/pressed/disabled/selected/maxed/etc. share palette, outline weight, density, and silhouette language.
...
## Project Bindings
reads: art task card from [[Architect]], [[GDD]] (art style section), `Docs/Visual-Design.md`
