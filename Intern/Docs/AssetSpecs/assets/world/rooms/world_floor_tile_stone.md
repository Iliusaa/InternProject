---
asset_id: world_floor_tile_stone
category: world_room
source_files:
  - STATUS.md
  - CONTEXT.md
...
# Asset: world_floor_tile_stone

## Purpose
Reusable stone floor tile.

## Gameplay or UI Context
Supports repeated or modular floor detail in combat rooms without making center gameplay noisy.

## Pixel Dimensions
| Variant or State | Width px | Height px | Notes | Source |
|---|---:|---:|---|---|
| tile | 128 | 128 | Tile sprite size. | inferred_from_layout |

## Placement and Anchor Rules
Pivot center. Tileable on floor grid. Avoid high-contrast detail in the combat plane.

## Visual Description
Cracked gray dungeon stone tile, uneven black outline, low-saturation scuffs, subtle grime, no strong symbols.

## States and Variants
- `default`: default state or variant
- `variant_a`: variant_a state or variant
- `variant_b`: variant_b state or variant

## Sprite Sheet or Export Plan
Export as PNG/RGBA with opaque background. Use one horizontal state sheet. Cell size: 128x128 px. Padding between cells: 16 px. Pivot: center. Target output filename: `world_floor_tile_stone_sheet.png`. Readability: must remain readable at 64x64 px on a phone screen. No baked dynamic text or numbers.

## Generation Prompt
Create `world_floor_tile_stone` for Stackspire as original 2D dark fantasy mobile game art, fixed 2D three-quarter top-down perspective where relevant, grotesque hand-drawn proportions, thick uneven black ink outlines, scuffed painterly dungeon texture, grim candlelit tower mood, mobile-readable silhouette. Subject: tileable cracked dungeon stone floor square, low contrast center detail. Intended use: Game HUD. Palette: ink void #0E0B10 #17131A #262129, dungeon stone #343038 #4B4447 #6D6461, Warrior dark crimson #6E151D #9E2527, damage red #D64A32 reserved only for damage and urgent feedback, torch and coin gold #8F5A24 #C18432 #E0B75C, Archer sickly green #35533B #587642 #8A9B55, Mage occult blue-violet #26364F #415C78 #7B6A9B, bone and parchment #B8A487 #D6C6A1. Background rule: opaque full-screen game background; do not use #00FF00 in final visuals unless it is a raw removable chroma-key background that will be removed before Unity import. Composition and readability: three-cell tile variant sheet or individual PNGs, opaque tile permitted. no readable text, no numbers, no watermark, do not copy existing game characters, UI symbols, logos, or proprietary designs.
