...
## Purpose
Reusable class-selection card art.

## Gameplay or UI Context
Frames Warrior, Archer, and Mage class content on Class Select. Unity text, icons, and silhouettes fill the card.

## Pixel Dimensions
| Variant or State | Width px | Height px | Notes | Source |
|---|---:|---:|---|---|
| default | 888 | 300 | Class card state cell. | Docs/UIUX_MVP_SPEC.md |
| selected | 888 | 300 | Class card state cell. | Docs/UIUX_MVP_SPEC.md |
| pressed | 888 | 300 | Class card state cell. | Docs/UIUX_MVP_SPEC.md |
| disabled | 888 | 300 | Class card state cell. | Docs/UIUX_MVP_SPEC.md |

## Placement and Anchor Rules
Cards sit in the Class Select vertical stack at x 96-984, y 220-1270 with 28 px gaps. Pivot center. Selected card may scale to 1.02 in Unity while sprite cell stays 888x300.

## Visual Description
Tall chipped stone card with accent strip area, thick ink outline, inner content zone, and state-specific border shapes. Selected state must use border/check/scale language, not color alone.

## States and Variants
- `default`: default state or variant
- `selected`: selected state or variant
- `pressed`: pressed state or variant
- `disabled`: disabled state or variant

## Sprite Sheet or Export Plan
Export as PNG/RGBA with transparent background. Use one horizontal state sheet. Cell size: 888x300 px. Padding between cells: 16 px. Pivot: center. Target output filename: `ui_class_card_portrait_sheet.png`. Readability: must remain readable at 64x64 px on a phone screen. Do not bake labels such as Start Run, Buy, Score, Coins, Pause, Restart, Upgrade, Room, prices, scores, health values, or room numbers into the sprite; all labels and numbers must remain Unity UI text.

## Generation Prompt
Create `ui_class_card` for Stackspire as original 2D dark fantasy mobile game art, fixed 2D three-quarter top-down perspective where relevant, grotesque hand-drawn proportions, thick uneven black ink outlines, scuffed painterly dungeon texture, grim candlelit tower mood, mobile-readable silhouette. Subject: tall class card stone frame with empty content area and class accent sockets. Intended use: Class Select. Palette: ink void #0E0B10 #17131A #262129, dungeon stone #343038 #4B4447 #6D6461, Warrior dark crimson #6E151D #9E2527, damage red #D64A32 reserved only for damage and urgent feedback, torch and coin gold #8F5A24 #C18432 #E0B75C, Archer sickly green #35533B #587642 #8A9B55, Mage occult blue-violet #26364F #415C78 #7B6A9B, bone and parchment #B8A487 #D6C6A1. Background rule: transparent background for a reusable sprite or UI element; do not use #00FF00 in final visuals unless it is a raw removable chroma-key background that will be removed before Unity import. Composition and readability: four card frames with transparent outside corners and no baked labels. no readable text, no numbers, no watermark, do not copy existing game characters, UI symbols, logos, or proprietary designs. Do not bake labels such as Start Run, Buy, Score, Coins, Pause, Restart, Upgrade, Room, prices, scores, health values, or room numbers into the sprite; all labels and numbers must remain Unity UI text.
